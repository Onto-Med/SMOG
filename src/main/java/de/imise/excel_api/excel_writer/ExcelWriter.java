...
//		setDateCellFormat(c);
	...
	public static void setDateValue(Row row, int colNum, String value) throws ParseException {
    	...
    	if (date.isPresent())
    		setValue(row, colNum, date.get());
    	else
    		throw new ParseException("Wrong date format: '" + value + "'!", 0);
	}
	...
	public static void setDateValue(Sheet sheet, int rowNum, int colNum, String value) throws ParseException {
	...
	public static void setValues(Sheet sheet, int rowNum, int colNum, String separator, String... values) {
		setValues(sheet.getRow(rowNum), colNum, separator, values);
    }

	public static void setValues(Sheet sheet, int rowNum, int colNum, String separator, int... values) {
		setValues(sheet.getRow(rowNum), colNum, separator, values);
	}

	public static void setValues(Sheet sheet, int rowNum, int colNum, String separator, double... values) {
		setValues(sheet.getRow(rowNum), colNum, separator, values);
	}

	public static void setValues(Sheet sheet, int rowNum, int colNum, String separator, Date... values) {
		setValues(sheet.getRow(rowNum), colNum, separator, values);
	}

	public static void setDateValues(Sheet sheet, int rowNum, int colNum, String separator, String... values) throws ParseException {
		setDateValues(sheet.getRow(rowNum), colNum, separator, values);
	}
	
	public static void setValues(Sheet sheet, int rowNum, int colNum, String separator, boolean... values) {
		setValues(sheet.getRow(rowNum), colNum, separator, values);
	}
...
//			cell.setCellStyle(cell.getRow().getSheet().getWorkbook().createCellStyle());
	...
	public static void clearTable(Sheet sheet, int firstRow, int firstCol, int lastCol) {
		for (int i = firstRow; true; i++) {
            if (ExcelReader.isEmptyRowPart(sheet.getRow(i), firstCol, lastCol))
                break;
            else {
	...
	public static void shiftTableRecordsDown(Sheet sheet, int firstRow, int firstCol, int lastCol) {
		for (int i = ExcelReader.getLastTableRowNum(sheet, firstRow, firstCol, lastCol); i >= firstRow; i--)
			copyRowPart(sheet, i, i+1, firstCol, lastCol);
	}
	...
	public static void shiftTableRecordsUp(Sheet sheet, int firstRow, int firstCol, int lastCol) {
		...
		for (int i = firstRow; i <= lastTabRowNum; i++)
			copyRowPart(sheet, i, i-1, firstCol, lastCol);
		
		clearRowPart(sheet, lastTabRowNum, firstCol, lastCol);
	}
	
	private static void copyRowPart(Sheet sheet, int oldRowNum, int newRowNum, int firstColNum, int lastColNum) {
		for (int i = firstColNum; i <= lastColNum; i++)
	...
	private static void updateCellReferencesForShifting(Sheet sheet, int rowNum) {
		for (int nRow = rowNum; nRow <= sheet.getLastRowNum(); ++nRow) {
			...
			if (row != null) {
				for (Cell c : row)
				   ((XSSFCell) c).updateCellReferencesForShifting(""); 
			}
		}
	}
