...
public class ExcelReader {
	...
	public static Optional<Integer> getIntegerValue(Cell cell) {
		...
		if (isString(cell))
			return StrUtil.parseInt(cell.getStringCellValue().trim());
	...
	public static Optional<Double> getDoubleValue(Cell cell) {
		...
		if (isString(cell))
			return StrUtil.parseDouble(cell.getStringCellValue().trim());
	...
	public static Optional<Date> getDateValue(Cell cell) {
		...
		if (isString(cell))
			return StrUtil.parseDate(cell.getStringCellValue().trim());
	...
	public static List<String> getStringValues(Sheet sheet, int rowNum, int colNum, String separator) {
		return getStringValues(sheet.getRow(rowNum), colNum, separator);
	}

	public static List<Integer> getIntegerValues(Sheet sheet, int rowNum, int colNum, String separator) {
		return getIntegerValues(sheet.getRow(rowNum), colNum, separator);
	}

	public static List<Double> getDoubleValues(Sheet sheet, int rowNum, int colNum, String separator) {
		return getDoubleValues(sheet.getRow(rowNum), colNum, separator);
	}

	public static List<Date> getDateValues(Sheet sheet, int rowNum, int colNum, String separator) throws ParseException {
		return getDateValues(sheet.getRow(rowNum), colNum, separator);
	}

	public static List<Boolean> getBooleanValues(Sheet sheet, int rowNum, int colNum, String separator) throws ParseException {
	...
	public static List<Date> getDateValues(Row row, int colNum, String separator) throws ParseException {
		List<Date> values = new ArrayList<>();
		for (String val : getValues(row, colNum, separator)) {
			Optional<Date> dateOpt = StrUtil.parseDate(val);
			if (dateOpt.isPresent())
				values.add(dateOpt.get());
			else
				throw new ParseException("Wrong date format: '" + val + "'!", 0);
		}
		
		return values;
	}
	
	public static List<Boolean> getBooleanValues(Row row, int colNum, String separator) throws ParseException {
		List<Boolean> values = new ArrayList<>();
		for (String val : getValues(row, colNum, separator)) {
			Optional<Boolean> boolOpt = StrUtil.parseBoolean(val);
			if (boolOpt.isPresent())
				values.add(boolOpt.get());
			else
				throw new ParseException("Wrong boolean format: '" + val + "'!", 0);
		}
...
//	}
	...
	public static Coordinates getValueCellCoordinatesForNameCell(Cell cell) {
		if (hasMark(cell, Entity.FIELD))
			...
			return new Coordinates(cell.getRowIndex() + 1, cell.getColumnIndex());
	}
	...
	public static String getDataType(Cell nameCell, Cell valueCell) {
		...
		else if (isInteger(valueCell))
			return DataType.INTEGER;
		else if (isDouble(valueCell) || isNumeric(valueCell))
			return DataType.DOUBLE;
		else if (isBoolean(valueCell))
	...
	private static Optional<String> getSpecifiedDataType(Cell cell) {
		...
		if (value.isPresent())
			return StrUtil.findInString(Entity.DATA_TYPE_PATTERN, value.get());
		else
			return Optional.empty();
	}
	...
	public static Tree getTree(Sheet sheet, int row, int col, HashMap<Integer, FreePositionTableRecord> table) {
	...
	public static int getLastTableRowNum(Sheet sheet, int firstRow, int firstCol, int lastCol) {
		...
		if (isEmptyRowPart(row, firstCol, lastCol))
			return firstRow - 1;
		
		for (int i = firstRow + 1; true; i++) {
            if (isEmptyRowPart(sheet.getRow(i), firstCol, lastCol))
                return i-1;
        }
	}
}
