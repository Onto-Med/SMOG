...
public class Sheet3 {
    ...
    public List<Sheet3Tab2Record> getTab2() {
        ...
        for (int i = 2; true; i++) {
            ...
            if (ExcelReader.isEmptyRowPart(row, 3, 4))
                break;
            else
                records.add(new Sheet3Tab2Record(row, 3, 4));
        }
        return records;
    }
    ...
    public List<Sheet3Tab1Record> getTab1() {
        ...
        for (int i = 2; true; i++) {
            Row row = sheet.getRow(i);
            if (ExcelReader.isEmptyRowPart(row, 0, 1))
                break;
            else
                records.add(new Sheet3Tab1Record(row, 0, 1));
        }
        return records;
    }
