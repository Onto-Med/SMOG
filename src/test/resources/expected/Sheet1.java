...
public class Sheet1 {
  ...
  public List<Sheet1Tab4Record> getTab4() {
    ...
    for (int i = 9; true; i++) {
      ...
      if (ExcelReader.isEmptyRowPart(row, 6, 10)) break;
      else records.add(new Sheet1Tab4Record(row, 6, 10));
    }
    return records;
  }
  ...
  public List<Sheet1Tab3Record> getTab3() {
    ...
    for (int i = 9; true; i++) {
      Row row = sheet.getRow(i);
      if (ExcelReader.isEmptyRowPart(row, 0, 4)) break;
      else records.add(new Sheet1Tab3Record(row, 0, 4));
    }
    return records;
  }
  ...
  public List<Sheet1Tab2Record> getTab2() {
    ...
    for (int i = 2; true; i++) {
      Row row = sheet.getRow(i);
      if (ExcelReader.isEmptyRowPart(row, 6, 9)) break;
      else records.add(new Sheet1Tab2Record(row, 6, 9));
    }
    return records;
  }
  ...
  public List<Sheet1Tab1Record> getTab1() {
    ...
    for (int i = 2; true; i++) {
      Row row = sheet.getRow(i);
      if (ExcelReader.isEmptyRowPart(row, 0, 4)) break;
      else records.add(new Sheet1Tab1Record(row, 0, 4));
    }
    return records;
  }
  ...
  public List<Sheet1Tab5Record> getTab5() {
    Optional<Coordinates> coord =
        ExcelReader.find(Entity.FREE_POSITION_TABLE, "Tab5", sheet);
    if (!coord.isPresent()) return null;
    ...
    for (int i = firstRowNum; true; i++) {
      Row row = sheet.getRow(i);
      if (ExcelReader.isEmptyRowPart(row, firstColNum, lastColNum)) break;
      else records.add(new Sheet1Tab5Record(row, firstColNum, lastColNum));
    }
    return records;
  }
  ...
  public boolean isEmptyTab5() {
    Optional<Coordinates> coord =
        ExcelReader.find(Entity.FREE_POSITION_TABLE, "Tab5", sheet);
    if (!coord.isPresent()) return true;
  ...
  public Sheet1Tab5Record addRecordInTab5() {
    ...
    return new Sheet1Tab5Record(
        ExcelWriter.getRow(sheet, lastRowNum + 1), firstColNum, lastColNum);
  }
